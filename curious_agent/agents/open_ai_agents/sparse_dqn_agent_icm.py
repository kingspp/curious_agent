...
from curious_agent.util.sparser_rewards_definitions import MakeRewardSparserThresholdBased
...
class SparseDQNAgentICM(Agent):
    ...
    def __init__(self, env, agent_config: Munch):
        ...
        super(SparseDQNAgentICM, self).__init__(env=env, agent_config=agent_config)
    ...
    def train(self):
        ...
        for i_episode in range(1, self.state.config.max_episodes + 1):
            ...
            while not done:
                ...
                reward = self.reward_filter(reward)  # making the reward sparser
